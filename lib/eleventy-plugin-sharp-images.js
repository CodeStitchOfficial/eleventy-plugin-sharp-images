...
const fsPromises = require('fs').promises; // Promisified file system operations
...
function createSharpPlugin(eleventyConfig, options = {}) {
    ...
    function hashConfig(config) {
        const { inputPath, operations } = config;
        const stats = fs.statSync(inputPath);
        const lastModified = stats.mtime.toISOString();
        const configString = JSON.stringify({ inputPath, operations, lastModified });
    ...
    fsPromises.mkdir(pluginOptions.outputDir, { recursive: true });

    // Return the plugin object with a method to clear the output directory
    return {
        clearOutputDir: async () => {
            imageCache.clear();
            await fsPromises.rm(pluginOptions.outputDir, { recursive: true, force: true });
            await fsPromises.mkdir(pluginOptions.outputDir, { recursive: true });
        }
    };
}
